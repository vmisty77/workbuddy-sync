...
- 该流程通常需要 host-owned PAT scope：`drive:upload-info` 和 `drive:commit`；如果未授权，会返回 `PAT_MEDIUM_RISK_NO_PERMISSION`，用户确认后可一次性执行 `dws pat chmod drive:upload-info drive:commit --agentCode workbuddy --grant-type once --format json` 再重放上传流程
...
# Step 0: 如未授权，用户确认后先补齐一次性 PAT 权限
# 钉盘上传包：drive:upload-info + drive:commit
...
- 钉盘常见 PAT scope：建文件夹 `drive:mkdir`，下载链接 `drive:download`，上传文件 `drive:upload-info` + `drive:commit`。可在用户确认后按工作流一次性授权，避免中途反复中断。
- `dws pat chmod --grant-type session` 当前需要 `--session-id`；没有 session id 时用 `--grant-type once`。
