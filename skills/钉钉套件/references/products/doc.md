...
Current CLI flags:
  ...
  dws doc update --node <DOC_ID> --content-file ./content.md --mode overwrite
Flags:
      --node string          文档 ID 或 URL (必填)
      --content string       Markdown 内容
      --content-file string  Markdown 文件路径，或 - 表示 stdin
      --mode string          更新模式: overwrite/replace=覆盖，append=追加
```

注意：2026-05-14 探针发现当前 `dws doc update` 的 CLI flag overlay（`content`/`content-file`）与后端 schema 必填字段 `markdown` 存在不一致，可能返回“缺少必要参数 markdown”。如果全文更新失败，优先使用 `doc block insert/update` 这组块级 API；全文更新问题需要同步上游 dws 修复。该操作命中 host-owned PAT scope：`doc:update`。
...
权限：2026-05-14 探针确认首次插入块会触发 host-owned PAT scope `doc.block:insert`。
